...
class CustomerController extends Controller
{
    ...
    {
        $checkemail = DB::table("customer")
            ->select("email")
            ->where("email", $req->email)
            ->first();
        $checkemail2 = DB::table("accounts")
            ->select("email")
            ->where("email", $req->email)
            ->first();
    ...
    public function login(Request $req)
    {
        ...
        if (
            $checkemail != null &&
            Hash::check($req->password, $checkemail->password)
        ) {
            $message = "Thành công";
            return response()->json([
                "status" => 200,
            ...
            $checkemail2 != null &&
            Hash::check($req->password, $checkemail2->password)
        ) {
            $message = "admin";
            return response()->json([
                "status" => 200,
    ...
    public function editprofile(Request $req)
    {
        ...
        $email = $req->email;
        $checkemail = DB::table("customer")
            ->where([["email", $email], ["customer_id", "!=", $req->id]])
            ->first();
        if ($checkemail != null) {
            return response()->json([
                "status" => 200,
    ...
    {
        $orderid = $req->orderid;
        $getorderdetail = DB::table("orderdetail")
            ->select(
