...
class ProductController extends Controller
{
    ...
    public function showproductbycate(Request $req)
    {
        ...
        // ]);
        //cach 2
        $query1 = DB::table("catesex")
            ->select("cateagesex_id")
            ->join(
                "category_detail",
                "catesex.catesex_id",
                "=",
                "category_detail.catesex_id"
            )
            ->where("catesex.category_id", $req->id);
        if ($req->sex != null) {
        ...
        foreach ($result as $values) {
            foreach ($values as $value) {
                $query2->orwhere("cateagesex_id", $value)->where("status", 1);
            }
        }
        $result2 = $query2->paginate(4);
        return response()->json([
            "status" => 200,
            "productcate" => $result2,
        ]);
